...
const Rg = () => {
  ...
  const [txtButton, setTxtButton] = useState("Gerar RG");
  const [statusButton, setStatusButton] = useState(false);
  const [opcao, setOpcao] = useState("sim");
  const [quantidade, setQuantidade] = useState(1);
  const [json, setJson] = useState(null);
  const url = "http://localhost:5000/generate?type=rg&quantity=" + quantidade;
  ...
  const formatarJsonRG = (arr) => {
    return arr.map(formatRG);
  };
  ...
  return (
    <div className="pagina">
      <h1>Gerador de RG</h1>
      <p>Gerador de RG válido utilizando validação do Orgão Emissor (SSP-SP)</p>
      <form onSubmit={handleSubmit}>
        ...
        {!error ? (
          <input
            className={txtButton === "Gerar RG" ? "ativo" : "inativo"}
            type="submit"
            value={txtButton}
            ...
            <textarea
              ...
              value={
                json === null
                  ? ""
                  : JSON.stringify(
                      opcao === "sim" ? formatarJsonRG(json) : json,
                      null,
                      2
                    )
              }
      ...
      <p style={{ color: "#525252" }}>
        IMPORTANTE: Nosso gerador online de RG tem como intenção ajudar
        estudantes, programadores, analistas e testadores a gerar RG válidos,
